...
export default function Menu() {
  ...
  return (
    <div className="min-h-screen pt-8" onMouseMove={createPawTrail}>
      {/* Paw Trails */}
      

      {/* Hero Section */}
      <section className="py-20 bg-gradient-to-br from-coffee-brown to-caramel text-cream-latte animate-fadeIn">
        <div className="container mx-auto px-4 text-center">
          <div className="mb-8 animate-bounce">
            <div className="w-32 h-32 mx-auto mb-6 bg-golden-fur rounded-full flex items-center justify-center shadow-lg transform transition-all hover:scale-110 hover:rotate-12">
              <Coffee className="text-coffee-brown animate-pulse" size={64} />
            </div>
          </div>

          <h1 className="font-baloo text-5xl lg:text-6xl font-bold mb-6 animate-slideInUp" data-testid="text-menu-title">
            Our Menu
          </h1>
          <p className="text-xl opacity-90 mb-8 max-w-3xl mx-auto animate-slideInDown" data-testid="text-menu-subtitle">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8 animate-fadeIn">
            <Badge className="bg-playful-teal text-coffee-brown px-6 py-2 text-lg shadow-md transform transition-all hover:scale-105" data-testid="badge-cost">₹500-1050 Cost for Two</Badge>
            <Badge className="bg-golden-fur text-coffee-brown px-6 py-2 text-lg shadow-md transform transition-all hover:scale-105" data-testid="badge-hours">7:30 AM - 11:30 PM All Days</Badge>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-cream-latte">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="font-baloo text-4xl lg:text-5xl font-bold text-coffee-brown mb-4 animate-slideInLeft" data-testid="text-matcha-title">
              Matcha Specials
            </h2>
            <p className="text-xl text-coffee-brown opacity-80 font-pacifico animate-slideInRight" data-testid="text-matcha-subtitle">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {matchaSpecials.map((item, index) => (
              <Card key={index} className="bg-white rounded-3xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105 paw-cursor group relative overflow-hidden" onMouseMove={createPawTrail}>
                <div className="absolute top-3 right-3 cursor-pointer z-10" onClick={() => toggleFavorite(item.name)}>
                  <Heart size={28} className={`transition-colors ${favorites.has(item.name) ? "fill-puppy-pink text-puppy-pink" : "fill-transparent text-gray-400"}`} />
                </div>
                <CardContent className="p-6">
                  <img
                    src={item.image}
                    alt={item.name}
                    className="w-full h-48 object-cover rounded-2xl mb-4 transform transition-all duration-300 group-hover:scale-105"
                  ...
                  <Button className="w-full bg-golden-fur text-coffee-brown hover:bg-opacity-90 paw-cursor transform transition-all hover:scale-105" data-testid={`button-add-cart-matcha-${index}`}>
                    ADD TO CART
                  </Button>
                </CardContent>
              </Card>
      ...
      {/* Paw Print Separator */}
      <div className="paw-separator my-8 animate-bounce"></div>

      {/* Main Menu */}
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="font-baloo text-4xl lg:text-5xl font-bold text-coffee-brown mb-4 animate-slideInLeft" data-testid="text-main-menu-title">
              Complete Menu
            </h2>
            <p className="text-xl text-coffee-brown opacity-80 animate-slideInRight" data-testid="text-main-menu-subtitle">
              Fresh • Made to Order ✨
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 xl:grid-cols-4 gap-8">
            {menuCategories.map((category) => (
              <Card key={category.id} className="bg-white rounded-3xl shadow-lg hover:shadow-xl transition-shadow transform hover:scale-105 relative overflow-hidden" data-testid={`card-category-${category.id}`} onMouseMove={createPawTrail}>
                <CardHeader className="text-center pb-6">
                  <div className={`${category.bgColor} p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center shadow-md transform transition-all hover:scale-110`}>
                ...
                <CardContent className="space-y-4">
                  {category.items.map((item, index) => (
                    <div key={index} className="flex justify-between items-start border-b border-cream-latte pb-3 last:border-b-0 transition-all duration-300 hover:bg-cream-latte/30 hover:ml-4" data-testid={`item-${category.id}-${index}`}>
                      ...
                      <div className="ml-4 flex flex-col items-end">
                        ...
                        <Coffee className="text-golden-fur mt-1 transform transition-all hover:rotate-12" size={16} />
                      </div>
                    </div>
                  ))}
                </CardContent>
      ...
      <section className="py-20 bg-cream-latte">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="font-baloo text-4xl font-bold text-coffee-brown mb-4 animate-slideInUp" data-testid="text-good-to-know-title">
              Good to Know
            </h2>
          </div>

          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <Card className="bg-white p-6 text-center shadow-lg transform transition-all hover:scale-105">
              <CardContent className="p-6">
            ...
            <Card className="bg-white p-6 text-center shadow-lg transform transition-all hover:scale-105">
              <CardContent className="p-6">
                ...
                <Button
                  asChild
                  className="bg-playful-teal text-coffee-brown hover:bg-opacity-90 paw-cursor transform transition-all hover:scale-105"
